...
from transt.ltr.admin.model_constructor import NetConstructor

import transt.ltr.admin.settings as ws_settings
...
def load_network(network_dir=None, checkpoint=None, constructor_fun_name=None, constructor_module=None, **kwargs):
    ...
    if 'constructor' in checkpoint_dict and checkpoint_dict['constructor'] is not None:
        ...
        if "transt." in net_constr.fun_module:
            net_fun = getattr(importlib.import_module(net_constr.fun_module), net_constr.fun_name)
        else:
            net_fun = getattr(importlib.import_module(f"transt.{net_constr.fun_module}"), net_constr.fun_name)
        ...
        if not "transt." in net_constr.fun_module:
            net_module = importlib.import_module(f"transt.{net_constr.fun_module}")
        else:
            net_module = importlib.import_module(net_constr.fun_module)

        net_fun = getattr(net_module, net_constr.fun_name)
