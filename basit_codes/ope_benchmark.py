...
class OPEBenchmark:
    ...
    def save_to_excel(self, success_ret, file_name, helight_threshold=0.5):
        """Save success result to excel file
        Args:
            result: returned dict from function eval
        """
        dct = {}
        t_name = list(success_ret.keys())[0]
        videos = list(success_ret[t_name].keys())
        dct["Video_Name"] = videos
        ...
        for tracker_name in success_ret.keys():
            ...
            for video in videos:
                ...
                tracker_succ.append(np.around(success, decimals=3))
            
            dct[f"{tracker_name}_Succ"] = tracker_succ
