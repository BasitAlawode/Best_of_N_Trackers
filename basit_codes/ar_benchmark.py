...
from toolkit.utils import calculate_accuracy
from basit_codes.calculate_failures import calculate_failures
...
class AccuracyRobustnessBenchmark:
    ...
    def show_result(self, result, eao_result=None, show_video_level=False, helight_threshold=0.5):
        ...
        # for tracker_name, ret in result.items():
            ...
            robustness = 1 - np.mean(np.sum(np.array(failures), axis=0) / length)
    ...
    def _calculate_accuracy_robustness(self, tracker_name):
        ...
        for i in range(len(self.dataset)):
            ...
            for tracker_traj in tracker_trajs:
                ...
                num_failures = calculate_failures(tracker_traj, gt_traj, fail_iou=0.5)[0]
    ...
    def save_to_excel(self, result, file_name, helight_threshold=0.5):
        """Save accuracy result to excel file
        Args:
            result: returned dict from function eval
        """
        dct = {}
        videos = list(result[list(result.keys())[0]]['overlaps'].keys())
        dct["Video Name"] = videos
        ...
        for tracker_name in result.keys():
            ...
            for video in videos:
                ...
                tracker_acc.append(np.around(accuracy, decimals=3))
            
            dct[f"{tracker_name} Acc"] = tracker_acc
        
        pd.DataFrame(dct).to_excel(file_name, index=False)
