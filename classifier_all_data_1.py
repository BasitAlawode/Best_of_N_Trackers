...
class Predictor(nn.Module):
    def __init__(self, num_classes=7) -> None:
        ...
        self.attention_map = nn.Conv2d(1, 64, kernel_size=(7, 7), 
                                       stride=(2, 2), 
                                       padding=(3, 3), bias=False)
        
        # Classification heads
        #self.dense_1 = nn.Linear(self.backbone.num_features, 128)
        #self.act_1 = nn.ReLU()
        ...
        self.dense_c1 = nn.Conv2d(self.backbone.num_features, num_classes,
                                  kernel_size=(1,1), stride=(1,1))
        self.act_c1 = nn.Softmax(dim=-1)
        
        self.dense_r1 = nn.Conv2d(self.backbone.num_features, num_classes,
                                  kernel_size=(1,1), stride=(1,1))
        self.act_r1 = nn.Sigmoid()
    ...
    def forward(self, x, roi_mask=None):
        ...
        y = self.backbone.stem.pool(y)
        y = self.backbone.stages(y)
        ...
        y = self.backbone.head.global_pool(y)
        y = self.backbone.head.drop(y)
        
        y1 = self.dense_c1(y)
        y1 = self.backbone.head.flatten(y1)
        class_prob = self.act_c1(y1)
        
        y2 = self.dense_r1(y)
        y2 = self.backbone.head.flatten(y2)
        ...
        for e in range(1, EPOCHS+1):
            ...
            for i in range(n_step):
                ...
                if (i%print_every==0) or ((i+1) == n_step):
                    print(f"   Epoch: {e}/{EPOCHS}, Step {i}/{n_step} >> \t Loss: {loss:.4f}, \t Acc: {acc:.4f}")
