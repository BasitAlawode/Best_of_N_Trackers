...
import importlib
import inspect
import ardimp.ltr.admin.settings as ws_settings
...
def load_network(network_dir=None, checkpoint=None, constructor_fun_name=None, constructor_module=None, **kwargs):
    ...
    if 'constructor' in checkpoint_dict and checkpoint_dict['constructor'] is not None:
        ...
        net_fun = getattr(importlib.import_module(f'ardimp.{net_constr.fun_module}'), net_constr.fun_name)
        ...
        #net = net_constr.get()
        net = net_fun(*net_constr.args, **net_constr.kwds)
    else:
...
def _setup_legacy_env():
    importlib.import_module('ardimp.ltr')
    sys.modules['dlframework'] = sys.modules['ardimp.ltr']
    sys.modules['dlframework.common'] = sys.modules['ardimp.ltr']
    importlib.import_module('ltr.admin')
    sys.modules['dlframework.common.utils'] = sys.modules['ardimp.ltr.admin']
    for m in ('model_constructor', 'stats', 'settings', 'local'):
        importlib.import_module('ardimp.ltr.admin.' + m)
        sys.modules['dlframework.common.utils.' + m] = sys.modules['ardimp.ltr.admin.' + m]
