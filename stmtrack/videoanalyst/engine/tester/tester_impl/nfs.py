...
from stmtrack.videoanalyst.evaluation.got_benchmark.experiments import ExperimentNfS
...
class NFSTester(TesterBase):
    ...
    extra_hyper_params = dict(
        device_num=1,
        data_root="datasets/NFS",
        fps=[30, ],  # (30|240)
    )

    def __init__(self, *args, **kwargs):
        super(NFSTester, self).__init__(*args, **kwargs)
    ...
    def test(self, ):
        ...
        for fps in self._hyper_params["fps"]:
            ...
            experiment = ExperimentNfS(root_dir=root_dir,
                                       fps=fps,
    ...
    def worker(self, dev_id, dev, fps, slicing_quantile):
        ...
        experiment = ExperimentNfS(root_dir,
                                   fps=fps,
                                   result_dir=result_dir,
                                   report_dir=report_dir)
...
NFSTester.default_hyper_params = copy.deepcopy(NFSTester.default_hyper_params)
NFSTester.default_hyper_params.update(NFSTester.extra_hyper_params)
